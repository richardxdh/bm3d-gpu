...
static inline void throw_if(bool cond, const char* msg)
{
...
int bm3d_denoise(
    const unsigned char* noisy,
    unsigned char* denoised,
...
)
{
	...
	//Allocate images
	CImg<unsigned char> image(noisy, w, h, 1, channels, true);
	CImg<unsigned char> image2(denoised, w, h, 1, channels, true);
    ...
    std::memcpy(denoised, image2.data(), image2.size());
